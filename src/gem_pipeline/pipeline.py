...
def _process_one(cfg, output_dir, dry_run=False, history_dir=None):
    ...
    # Apply survey-specific pre-processing via registry
    from gem_pipeline.ingestion.registry import get_preprocessor
    preprocessor = get_preprocessor(cfg.survey_name)
    if preprocessor is not None:
        logger.info("  [" + cfg.survey_id + "] Applying preprocessor for: " + cfg.survey_name)
        df = preprocessor(df, cfg)
